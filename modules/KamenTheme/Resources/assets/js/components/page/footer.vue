<template>
  <div class="bg-gradient-to-r from-pink-500 to-pink-600 text-white">
    <div
      class="
        mx-auto
        mt-10
        flex
        max-w-7xl
        flex-col
        divide-y divide-pink-700/50
        py-10
        px-4
        text-center
        lg:px-0
      "
    >
      <div class="grid grid-cols-12 gap-4 py-10">
        <div
          class="
            col-span-12
            row-end-3
            mt-8
            text-left
            md:col-span-4 md:row-end-1 md:mt-0
          "
        >
          <h1 class="mb-2 text-4xl font-bold">Tentang Kami</h1>
          <p class="leading-6">
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptas
            possimus labore ut accusamus maxime. A consequatur, officiis, veniam
            voluptatum voluptates quae ipsum provident nobis cum perferendis hic
            ducimus nihil neque dolores itaque atque perspiciatis quos magnam
            repellat. Ratione, inventore totam a laudantium quibusdam velit
            eveniet nam dolorum molestiae. Quisquam, nemo?
          </p>
        </div>
        <div class="col-span-12 md:col-span-8 ml-12">
          <div class="grid grid-cols-3 gap-4 text-left">
            <ul class="col-span-full md:col-span-1">
              <h1 class="pb-4 font-bold">Alamat</h1>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
            </ul>
            <ul class="col-span-full md:col-span-1">
              <h1 class="pb-4 font-bold">Pintasan</h1>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
            </ul>
            <ul class="col-span-full md:col-span-1">
              <h1 class="pb-4 font-bold">Media Sosial</h1>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
              <li class="mt-2">
                <a href="#" class="transition-all hover:text-gray-200">Jalan</a>
              </li>
            </ul>
          </div>
        </div>
      </div>
      <div class="grid grid-cols-12 gap-4 py-10">
        <div class="col-span-12 place-self-center md:col-span-4">
          Copyrigth &copy; {{ new Date().getFullYear() }} Kamen Theme - Lamaau
        </div>
      </div>
    </div>
  </div>
</template>
